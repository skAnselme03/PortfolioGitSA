...
function calculePaie() {
    ...
    let fTauxEtDemie = (fTauxSaisie * (1 + .5)).toFixed(2);
    let fTauxDouble = (fTauxSaisie * 2).toFixed(2);
    ...
    if (saisieValide(bSaisiValide, objResultat, objResultatC)) {
        ...
        if (ftotHNormal > NB_TOT_HEURES_NORMAL_SEM) {
            ...
            ftotHNormal = (NB_TOT_HEURES_NORMAL_SEM).toFixed(2);
        }
        // Traitement heures temps et demi
        if (ftotHWeekEnd[0] > 0) {
        ...
        //Listes des taux horaire
        fListeTauxHoraire = [fTauxSaisie, fTauxEtDemie, fTauxDouble];
        ...
        fSalaireBrut = [(ftotHNormal * fListeTauxHoraire[0]).toFixed(2),
            (ftotHTempsEtDemi * fListeTauxHoraire[1]).toFixed(2),
            (ftotHTempsDouble * fListeTauxHoraire[2]).toFixed(2)
        ];
        // Salaire brut total travaillées
        fTotSalaireBrut = sommeListe(fSalaireBrut);
        // Calculer les déductions
        fDeductionsPaie = calculDeductionEtImpots(fTotSalaireBrut);
        // Afficcher les résultats
        afficheValTabInLabel(objResultat, ftotHeures, 'lblHeures', '');
        afficheValTabInLabel(objResultat, fListeTauxHoraire, 'TauxHoraire', SIGLE_DEVISE);
...
function calculDeductionEtImpots(fSalaireBrutTot) {
    ...
    if (fSalaireBrutTot >= 450) {
        ...
        if (fSalaireBrutTot >= 350) {
            fcalcImpotsFed = TAUX_IMP_FED[1] +
                TAUX_IMP_FED[2] * (fBrutReajuste - 350);
    ...
    if (fSalaireBrutTot >= 350) {
        ...
        if (fSalaireBrutTot >= 250) {
            fcalcImpotsProv = TAUX_IMP_PROV[1] +
                TAUX_IMP_PROV[2] * (fBrutReajuste - 250)
