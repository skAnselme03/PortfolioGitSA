	...
	class ConnectBDD
	{
		...
		*/
		...
		public function selectData($requete, $parametres = array()) {
			...
			if (!empty($parametres)) {
				// Chaîne de caractères représentant les types des paramètres (ici, tous des chaînes de caractères)
				$types = str_repeat("s", count($parametres)); 
				// Lie les paramètres à la requête
				$statement->bind_param($types, ...$parametres); 
			}
		...
		//
		 ...
		 if (!empty($parametres)) {
			 // Chaîne de caractères représentant les types des paramètres
			 $types = str_repeat("s", count($parametres));
			 $bindParams = array();
	 
			 // Crée un tableau des paramètres avec les types
			 $bindParams[] = &$types;
			 foreach ($parametres as $key => $value) {
				 $bindParams[] = &$parametres[$key];
				 /* echo $parametres[$key] ."<br>"; */
				 echo $key ."<br>";
				 echo $value;
			 }
	 
			 // Lie les paramètres à la requête en utilisant la fonction call_user_func_array()
